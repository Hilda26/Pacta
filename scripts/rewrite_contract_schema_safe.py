...
CONTRACT = '''# { "Depends": "py-genlayer:1jb45aa8ynh2a9c9xn3b7qqh8sm5q93hwfp7jqmwsfhh8jpz09h6" }
...
CONTRACT_API = """# Pacta Covenant Registry Contract API
...
GENLAYER_DOC = """# GenLayer Integration

## Official Guidance Used

Pacta's contract follows current GenLayer docs and Skills patterns:

- A single Python `class Contract(gl.Contract)`.
- Public methods decorated with `@gl.public.view`, `@gl.public.write`, or `@gl.public.write.payable`.
- Persistent fields declared in the contract class body.
- Primitive public method parameters and return types where possible.
- JSON strings for complex read payloads to avoid Studio schema-loading failures.
- `TreeMap` for persistent mappings.
- GEN value transfers through payable methods, `gl.message.value`, and transfer interfaces.
- Nondeterministic LLM and web operations inside nondeterministic execution only.
- Deterministic state writes only after consensus returns.

## StudioNet Deployment

Use GenLayer StudioNet, not Docker-based GenLayer Studio deployment, for the project deployment target.

```bash
genlayer network studionet
genlayer deploy --contract contracts/pacta_covenant_registry.py --args 1000000000000000000
```

The constructor argument is `min_bond_wei`. The example above sets the minimum bond to `1 GEN`.

After deployment, provide the deployed address and set:
...
The contract exposes `get_event_count()` and `get_event(event_id)`. Backend workers can poll these views and mirror events into PostgreSQL through the internal `/contract-events` endpoint.
"""
...
FILES = {
    "contracts/pacta_covenant_registry.py": CONTRACT,
    "contracts/CONTRACT_API.md": CONTRACT_API,
    "contracts/tests/test_contract_static.py": TESTS,
    "docs/GENLAYER.md": GENLAYER_DOC,
...
def main() -> None:
    ...
    print(f"Rewrote Pacta contract using schema-safe GenLayer patterns in {len(FILES)} files.")
